...
def main():
    ...
    solar_js=(ROOT/'data/solar-storage.js').read_text(encoding='utf-8');[(_ for _ in ()).throw(AssertionError(x)) if x not in solar_js else None for x in ('solarThreshold','z<8?25','z<9.5?10','z<11?5','z<12.5?2','MAX_VISIBLE_SOLAR')]
    ...
    for fragment in ('ENTSO-E aligned','ned_load_mw','ned_generation_mw','apply_ned_fallback','NED per-source detail as a comparison'):assert fragment in overlay,fragment
    ...
    index=(ROOT/'index.html').read_text(encoding='utf-8');assert 'data/headline-freshness.js' in index
