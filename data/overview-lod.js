...
function capture(){
 ...
 if(found>0){captured=true;totalConnections=found}
}
function shouldShow(layer,detail,active,flowOn,cableOn){const kv=kvOf(layer.feature||layer.__flowFeature);if(!active.has(kv))return false;if(!detail&&!overviewVoltage(kv))return false;if(layer.__lodKind==='flow')return flowOn&&(!layer.__flowCable||cableOn);if(layer.__lodKind==='cable')return cableOn;return true}
function sync(){
 ...
 for(const l of lowVoltageLayers){const show=shouldShow(l,detail,active,flowOn,cableOn);if(show){if(!map.hasLayer(l))l.addTo(map)}else if(map.hasLayer(l))map.removeLayer(l)}
 if(typeof flowPaths!=='undefined')for(const l of flowPaths){const kv=kvOf(l.__flowFeature);if(!overviewVoltage(kv))continue;const enabled=active.has(kv)&&flowOn&&(!l.__flowCable||cableOn);l.setStyle({opacity:enabled?.96:0,weight:enabled?Math.max(3,typeof lineWeight==='function'?lineWeight(l.__flowFeature.properties):3):0})}
