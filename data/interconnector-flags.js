(()=>{
  ...
  function flagPopup(ic){const known=hasFlow(ic.country),mw=known?Number(lastLive.border_flows[ic.country]):0,cap=countryCapacity(ic.country),into=mw>=0,pct=known&&cap?Math.round(Math.abs(mw)/cap*100):null;const src=(lastLive&&lastLive.border_flow_sources&&lastLive.border_flow_sources[ic.country])||'ENTSO-E';const explain=ic.country==='DE'?'ENTSO-E publiceert de actuele NL–DE stroom als totaal voor de marktgrens. De verdeling over de vier fysieke corridors is daarom afgeleid.':ic.country==='GB'?'ENTSO-E publiceert sinds Brexit geen NL–VK-stroom; deze waarde komt van Elexon/BMRS (GB-zijde, half-uurlijks).':ic.type?.includes('HVDC')?'Dit is een afzonderlijke HVDC-interconnector.':'De actuele waarde geldt voor de marktgrens.';return `<b>${flagSvg(ic.country,18,true)}${countryName[ic.country]||ic.country}</b><small>${ic.name} · ${ic.type}</small><small>${known?`${Math.abs(mw).toLocaleString('nl-NL')} MW · ${into?'import naar Nederland':'export uit Nederland'}`:'Geen actuele grensflow beschikbaar'}</small><small>${cap?`${cap.toLocaleString('nl-NL')} MW totale grenscapaciteit`:''}${pct!=null?` · ${pct}% benut`:''}</small><small style="color:${known?colorFor(ic.country,mw):UNKNOWN};font-weight:750">${known?'kaartkleur = benutting':'grijs = geen actuele flow'}</small><small class="provenance-line provenance-${known?'measured':'static'}">${known?'gemeten · '+src:'statisch · configuratie'}</small><small class="temporal-line temporal-${known?'actual':'none'}">${known?'gerealiseerd interval':'geen tijdreeks'}</small><small>${explain}</small><small>${ic.note||''}</small>`}
