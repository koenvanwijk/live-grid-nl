...
const style=document.createElement('style');style.textContent='.leaflet-overlay-pane path.province-flow-line{stroke-linecap:round!important;filter:drop-shadow(0 0 2px rgba(255,214,90,.28))}';document.head.appendChild(style);
...
function movingDot(a,b,mw,generation){const r=Math.max(2.2,Math.min(4.5,1.7+Math.sqrt(mw)/16)),dot=L.circleMarker(a,{radius:r,weight:0,fillOpacity:.95,fillColor:'#ffd65a',interactive:false}).addTo(map);layers.push(dot);let t=Math.random(),last=performance.now();const tick=now=>{if(generation!==animationGeneration||!map.hasLayer(dot))return;const dt=Math.min(50,now-last);last=now;t=(t+dt*(0.000055+Math.min(mw,1000)*0.000000012))%1;dot.setLatLng([a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t]);requestAnimationFrame(tick)};requestAnimationFrame(tick)}
function draw(){
 ...
 for(const f of routeRedistribution(balance)){const a=CENTERS[f.from],b=CENTERS[f.to],line=L.polyline([a,b],{color:'#ffd65a',weight:weight(f.mw),opacity:.56,dashArray:'5 9',interactive:true,className:'province-flow-line'}).bindPopup(flowPopup(f),{className:'grid-popup'}).addTo(map);layers.push(line);movingDot(a,b,f.mw,generation)}
