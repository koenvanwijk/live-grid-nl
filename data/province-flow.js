...
const FLOW_COLOR='#b56cff';
const layers=[];let lastData=null,demandModel=null,animationGeneration=0;
const style=document.createElement('style');style.textContent='.leaflet-overlay-pane path.province-flow-line{stroke-linecap:round!important;filter:drop-shadow(0 0 2px rgba(181,108,255,.32))}';document.head.appendChild(style);
function clear(){animationGeneration++;while(layers.length){const x=layers.pop();if(map.hasLayer(x))map.removeLayer(x)}}
...
function movingDot(a,b,mw,generation){const r=Math.max(2.2,Math.min(4.5,1.7+Math.sqrt(mw)/16)),dot=L.circleMarker(a,{radius:r,weight:0,fillOpacity:.95,fillColor:FLOW_COLOR,interactive:false}).addTo(map);layers.push(dot);let t=Math.random(),last=performance.now();const tick=now=>{if(generation!==animationGeneration||!map.hasLayer(dot))return;const dt=Math.min(50,now-last);last=now;t=(t+dt*(0.000055+Math.min(mw,1000)*0.000000012))%1;dot.setLatLng([a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t]);requestAnimationFrame(tick)};requestAnimationFrame(tick)}
function draw(){
 clear();if(!lastData||!demandModel||!visible())return;const generation=animationGeneration;
 ...
 for(const f of routeRedistribution(balance)){const a=CENTERS[f.from],b=CENTERS[f.to],line=L.polyline([a,b],{color:FLOW_COLOR,weight:weight(f.mw),opacity:.62,dashArray:'5 9',interactive:true,className:'province-flow-line'}).bindPopup(flowPopup(f),{className:'grid-popup'}).addTo(map);layers.push(line);movingDot(a,b,f.mw,generation)}
 for(const n of names){const bucket=p[n];if(!bucket)continue;const dm=Number(demand[n]||0),expected=totalRenewable*(dm/totalLoad),net=balance[n],s=Math.max(8,Math.min(16,8+Math.sqrt(Math.abs(net))/5)),m=L.circleMarker(CENTERS[n],{radius:s/2,color:FLOW_COLOR,weight:1,fillColor:net>=0?'#70e58b':'#ffb454',fillOpacity:.9,interactive:true}).bindPopup(nodePopup(n,bucket,dm,expected),{className:'grid-popup'}).addTo(map);layers.push(m)}
}
